    ...
    while (1) {
        send(client_socket, welcome_message, strlen(welcome_message), 0);
            ...
            if (strncmp(buffer, "login", 5) == 0) {
                handle_manager_session(client_socket);  
            }
            else if (strncmp(buffer, "logout", 6) == 0) {
                manager_logout(client_socket);  
            }else if (strncmp(buffer, "status", 6) == 0) {
                check_manager_status(client_socket);  
            ...
            pthread_exit(NULL);
                ...
                change_passwordManager(client_socket);  
            }else if (strncmp(buffer, "4", 1) == 0) {
                change_customerActivation(client_socket);  
            }
            else if (strncmp(buffer, "5", 1) == 0) {
                assign_applicationToEmployee(client_socket);  
            }else if (strncmp(buffer, "8", 1) == 0) {
                view_appManager(client_socket);  
            }else if (strncmp(buffer, "6", 1) == 0) {
                review_feedback(client_socket);  
            }else if (strncmp(buffer, "9", 1) == 0) {
                feedback_historyManager(client_socket);  }
        else {
